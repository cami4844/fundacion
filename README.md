...
- El panel funciona igual en el dominio real: https://tudominio.com/admin/ (cookie Secure
  automática con HTTPS; guía aparte: DESPLIEGUE-HOSTINGER.md).
...
- **Guías aparte** (en tu PC: `Downloads\_guias_fundacion_red_con_ciencia`, fuera del sitio):
...
2. Doble clic a `iniciar-servidor.bat` (Windows). También: `node server.js`.
...
index.html …           → el sitio público (6 páginas + 404)
admin/admin.html       → panel del equipo (noticias, mensajes, correo, seguridad)
...
Guía completa en `DESPLIEGUE-HOSTINGER.md` (carpeta de guías de tu PC:
VPS de Hostinger + dominio + https + arranque automático). El sitio también
funciona sin servidor (GitHub Pages) en "modo archivo": noticias del archivo
js/data/noticias.js y formulario por WhatsApp. El panel con bandeja, correos
y noticias guardadas requiere Node.js (VPS).
