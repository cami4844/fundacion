...
(async () => {
  ...
  const r = await env("Runtime.evaluate", { expression: `(()=>{
    ...
    f.querySelectorAll("*").forEach(el => {
      if (el.scrollWidth > f.clientWidth + 2 || el.offsetLeft + el.offsetWidth > f.clientWidth + 2 || getComputedStyle(el).position === "absolute")
        filas.push(el.tagName + "." + String(el.className).slice(0,28) + " sw=" + el.scrollWidth + " ow=" + el.offsetWidth + " ol=" + el.offsetLeft + " pos=" + getComputedStyle(el).position + " txt=" + (el.textContent||"").trim().slice(0,40));
    });
