(() => {
  ...
  async function reanudar() {
    ...
    if (r.ok && j.ok) { S.csrf = j.csrf; S.version = j.version; S.clavePorDefecto = j.clavePorDefecto; await entrar(); return; }
    /* Copia estática (GitHub Pages u otro hosting sin API): la respuesta
       no es JSON porque no hay servidor detrás. En vez de un formulario
       que nunca va a entrar, se explica la situación de una vez. */
    if (j.noJson) {
      $("#form-login").style.display = "none";
      const caja = document.getElementById("sin-servidor");
