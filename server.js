...
const CSP = [
  "default-src 'self'",
  "script-src 'self' 'sha256-LcBSGwcfizPCwZdNeY1i5vXYydAsZXVJ949s+LgG0Tk='",  /* el ajuste de rutas del 404 (solo ese bloque exacto) */
...
const NO_SERVIR = new Set([
  "server.js", "llaves-webauthn.js", "iniciar-servidor.bat",
  "cambiar-clave.js", "esquema.sql", "arrancar-servidor-oculto.vbs",
]);
...
function servirEstatico(req, res, urlPath) {
  ...
  /* el panel vive en admin/admin.html (pedido de Cami: no se llama index):
     /admin y /admin/ lo sirven igual, para que el enlace bonito siga vivo */
  if (ruta === "/admin" || ruta === "/admin/") ruta = "/admin/admin.html";
  ...
  fs.stat(abs, (err, st) => {
    if (err || !st.isFile()) {
      /* Solo se acepta el index de una CARPETA. Ya no se cae al index.html
         de la raíz: eso hacía que cualquier dirección inventada devolviera
         la portada con estado 200, y ni se veía la página de error ni
         Google entendía que la página no existía. */
    ...
    /* blindaje: la carpeta data/ (base, clave cifrada, esquema) NO se sirve.
       Ojo: con separador al final — sin él, "data-publica" también caería
       en el bloqueo por el prefijo en común. */
    const carpetaData = path.join(RAIZ, "data") + path.sep;
    if (String(abs) === path.join(RAIZ, "data") || String(abs).startsWith(carpetaData)) {
    ...
    /* ojo con la barra: ruta viene como "/admin/admin.html", sin ella el
       startsWith("admin/") nunca hacía match y el panel perdía sus
       cabeceras de no-index / no-referrer desde el renombre */
...
servidor.listen(PUERTO, () => {
  console.log("");
  console.log("  Fundacion Red Con Ciencia — servidor local (panel /admin/)");
  console.log("  ------------------------------------------");
